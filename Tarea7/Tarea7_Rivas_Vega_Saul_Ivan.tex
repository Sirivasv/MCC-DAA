...
\begin{algorithm}[H]
	...
	\SetAlgoLined
	\tcc{Inicializamos los arreglos para correr el algoritmo de Bellman-Ford, con la variación de que la distancia en lugar de ser $\infty$, ahora es $0$.}
	\ForEach{$v\in V$}{
		distancia[$v$] = 0\;
		padre[$v$] = -1\;
	}
	\tcc{Realizamos $n$ 'relajaciones' como en el algoritmo de Bellman-Ford, solo que realizamos $n$ en lugar de $n-1$,  si existió algún nodo que disminuyo su distancia y además era la $n$-ésima relajación, lo guardamos pues es parte de un ciclo negativo.}
	...
	\For{i = 1, i $\leq$ $n$}{
		\ForEach{$(u,v, peso) \in E$}{
			\If{$distancia[u] + peso < distancia[v]$}{
				...
				padre[v] = u\;
				\If{i = n}{
					$nodo\_en\_ciclo\_negativo=i$\;
				} 
			}
		}
	}
	\tcc{Revisamos si es que en efecto existió dicho nodo que disminuyo su distancia en la $n$-ésima relajación.}
	...
	\caption{Algoritmo CheckNegativeCycle.}
\end{algorithm}
